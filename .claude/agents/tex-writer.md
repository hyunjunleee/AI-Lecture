...
긴 전개(2줄 이상 display)나 비자명한 계산은 **무엇을 왜 계산하며 각 단계가 왜 적법한지**를 본문 산문으로 설명한다 — 전공 교과서·논문처럼 수식 사이를 논리로 부드럽게 잇는다. 특히 극한·적분·급수·미분의 교환은 그 근거(지배수렴/단조수렴/Fubini–Tonelli/지배함수, 가측성, 적분가능성)를 문장으로 댄다. 측도가 관련되면 어느 측도에 대한 것인지 밝힌다(사양서 §2.6A-3).
