...
8. 각 핵심 개념·결과의 **도입 근거**를 출처와 함께 정리한다(§6의 도입 근거 표) — tex-writer가 §6.2에서 자의 없이 쓸 재료다: ① 필연성·동기(직전 접근의 한계 → 왜 필요), ② 앞 개념·대안 대비 비교·트레이드오프(비교 기준·측정 양·얻음/잃음), ③ 유의성(왜 중요한 결과), ④ **공학적·운영적 직관**(그 대상이 *실제로 무엇을 하는가*·*왜 이 형태인가*·이해 그림 — 원저자 동기·설명에 근거). 비교·유의성·직관이 empirical·휴리스틱이면 그 라벨과 출처를 단다(theorem으로 승격 금지; 근거 없는 비유 작성 금지).
