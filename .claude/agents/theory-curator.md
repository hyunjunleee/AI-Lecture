...
8. 각 핵심 개념·결과의 **도입 근거**를 출처와 함께 정리한다(§5 도입 근거 표) — tex-writer가 §6.2에서 자의 없이 쓸 재료다: ① 필연성·동기(직전 접근의 한계 → 왜 필요), ② 앞 개념·대안 대비 비교·트레이드오프(비교 기준·측정 양·얻음/잃음), ③ 유의성(왜 중요한 결과), ④ **공학적·운영적 직관**(그 대상이 *실제로 무엇을 하는가*·*왜 이 형태인가*·이해 그림 — 원저자 동기·설명에 근거). 비교·유의성·직관이 empirical·휴리스틱이면 그 라벨과 출처를 단다(theorem으로 승격 금지; 근거 없는 비유 작성 금지).
9. **중심·load-bearing 방법**에 대해 **방법 완결 항목**(§6 완결 표)을 — **근거가 있을 때만**, 출처·라벨과 함께 — 정리한다(tex-writer가 §6.2 완결 작성에 쓸 재료): ⓐ 손실·목적함수·update equation·fixed-point, ⓑ 효과·성능(empirical이면 출처+라벨) ↔ 한계·실패모드(언제 깨지나·무엇을 못 푸나), ⓒ 이론적 렌즈(들)와 *어디까지 맞고 어디서 깨지나*, ⓓ 현대 foundation model에서의 쓰임(empirical+출처), ⓔ open problems(미해결을 *진술한 출처*에 근거). **근거가 없으면 빈칸으로 두고 지어내지 않는다**(필요하면 researcher 보강 요청). 직관·비유는 특정 reference에 등장할 때 그 형태를 손상·강화하지 않고 충실히 옮길 때만 재료로 둔다(자작 비유 금지).
...
| 개념/결과 | 필연성·동기 (직전 한계 → 왜 필요) | 비교·트레이드오프 (대상, 측정 양, 얻음/잃음) | 유의성 (왜 중요) | 공학적·운영적 직관 (무엇을 하는가·왜 이 형태·그림; 원저자 설명 근거) | provenance/출처 (empirical이면 라벨) |
|---|---|---|---|---|---|

## 6. 방법 완결 항목 (중심·load-bearing 방법, **근거 있을 때만** 채움 — 없으면 빈칸, 지어내지 말 것)

| 방법/결과 | 손실·목적·update/fixed-point | 효과·성능 (empirical+출처+라벨) | 한계·실패모드 (언제 깨지나) | 이론 렌즈(들) + 어디서 멈추나 | 현대 모델 쓰임 (empirical+출처) | open problems (진술 출처) |
|---|---|---|---|---|---|---|

각 칸은 **출처·라벨**(정리/empirical/사변 구분)을 단다. 직관·비유는 특정 reference에 충실할 때만(C2). 근거 없는 칸은 비워 두고, 필요하면 researcher 보강을 요청한다.
```
