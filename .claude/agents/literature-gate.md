...
원문, `research.md`, `theory.md`, `lecture.tex` 사이의 claim fidelity를 감사한다. 문서 내부 수학의 전수 감사는 `read-gate` / `claim-gate` / `model-gate`에게 맡긴다.
...
- 내부 수학 증명의 독해 가능성은 `read-gate` / `claim-gate` / `model-gate` 판정으로 넘긴다.
- 장문 직접 인용 금지. 원문 위치와 짧은 anchor만 남긴다.
- 과장 금지(fidelity)는 **개념마다 한 번의 정확한 범위 진술**로 충족된다 — 같은 "보장 아님/theorem 아님" hedge가 반복되어야 PASS인 것이 아니다. 한 번 충실히 밝혔으면 족하다. 반대로 그 경계가 *아예 없거나 원문보다 강하면* stall. (반복 hedge로 면책문이 된 문체 자체는 read-gate 소관.)
